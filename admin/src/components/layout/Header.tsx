    ...
    return (
        <header className="h-16 bg-white dark:bg-slate-900 border-b border-gray-200 dark:border-slate-700 flex items-center justify-between px-4 gap-4 transition-colors duration-300">
            ...
            <div className="flex items-center gap-4">
                <Button variant="ghost" size="icon" onClick={onMenuClick} className="lg:hidden dark:text-gray-300 dark:hover:bg-slate-800">
            ...
            <div className="flex items-center gap-2">
                ...
                {/* Language Toggle */}
                <Button variant="ghost" size="icon" onClick={toggleLanguage} className="dark:text-gray-300 dark:hover:bg-slate-800">
                    <Globe className="h-5 w-5" />
                </Button>

                {/* Notifications */}
                <Button variant="ghost" size="icon" className="relative dark:text-gray-300 dark:hover:bg-slate-800">
                ...
                <div className="flex items-center gap-3 ps-3 border-s border-gray-200 dark:border-slate-700">
                    <div className="hidden sm:block text-end">
                        ...
                        <p className="text-xs text-gray-500 dark:text-gray-400">{user?.department || 'Admin'}</p>
                    </div>
                    <Avatar>
                        <AvatarImage src={user?.avatar} alt={user?.name} />
                        <AvatarFallback className="dark:bg-slate-700 dark:text-gray-200">{user?.name ? getInitials(user.name) : 'AD'}</AvatarFallback>
                    </Avatar>
                    <Button variant="ghost" size="icon" onClick={logout} className="dark:text-gray-300 dark:hover:bg-slate-800">
