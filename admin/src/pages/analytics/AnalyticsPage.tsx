...
export function AnalyticsPage() {
  ...
  // Use topProducts from query or fallback to stats.topProducts (dashboard returns products by soldCount)
  const displayTopProducts =
    Array.isArray(topProducts) && topProducts.length > 0
      ? topProducts
      : stats?.topProducts && Array.isArray(stats.topProducts)
      ? stats.topProducts
      : [];

  // Top customers come from dashboard stats (customer-report API returns newCustomers/returningCustomers, not topCustomers)
  const displayTopCustomers =
    stats?.topCustomers && Array.isArray(stats.topCustomers)
      ? stats.topCustomers
      : [];
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="font-medium text-gray-900 dark:text-gray-100">
                          {customer.shopName || customer.companyName || "-"}
                        </p>
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-500">
                        {product.sales ?? product.soldCount ?? 0} وحدة
                      </p>
                    </div>
                  </div>
                  <p className="font-medium">
                    {formatCurrency(
                      product.revenue || product.basePrice || 0,
                      "SAR",
                      locale
                    )}
                  </p>
