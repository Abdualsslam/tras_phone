...
export function PurchaseOrdersPage() {
  ...
  const [receiveItems, setReceiveItems] = useState<
    { itemId: string; receivedQuantity: number; damagedQuantity: number }[]
  >([]);
  ...
  const createMutation = useMutation({
    ...
    onError: (error) =>
      toast.error(getErrorMessage(error, "حدث خطأ أثناء إنشاء أمر الشراء")),
  });
  ...
  const receiveMutation = useMutation({
    ...
    onError: (error) =>
      toast.error(getErrorMessage(error, "حدث خطأ أثناء تسجيل الاستلام")),
  });
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4 text-sm">
                ...
                <div>
                  ...
                  <Badge
                    variant={
                      statusConfig[selectedOrder.status]?.variant || "default"
                    }
                  >
