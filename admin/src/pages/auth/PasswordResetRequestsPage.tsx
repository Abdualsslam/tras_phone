...
export function PasswordResetRequestsPage() {
    ...
    const { data: requestsData, isLoading, error } = useQuery({
        ...
        queryFn: () => authApi.getPasswordResetRequests({ 
    ...
    const processMutation = useMutation({
        ...
        onSuccess: (result) => {
            ...
            toast.success('تم معالجة الطلب بنجاح. يرجى نسخ كلمة المرور وإرسالها للعميل.');
        },
        onError: () => {
    ...
    const rejectMutation = useMutation({
        mutationFn: ({ id, data }: { id: string; data: { rejectionReason: string; adminNotes?: string } }) =>
    ...
    const onRejectSubmit = (data: { rejectionReason: string; adminNotes: string }) => {
        if (selectedRequest) {
        ...
        <div className="space-y-6">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    ...
                    <p className="text-muted-foreground text-sm">معالجة طلبات إعادة تعيين كلمة المرور من العملاء</p>
                </div>
            </div>
                    ...
                    {selectedRequest && (
                        <div className="space-y-4">
                            <div className="grid grid-cols-2 gap-4 text-sm">
                                ...
                                {selectedRequest.processedBy && typeof selectedRequest.processedBy === 'object' && (
                                    <div>
                                        <p className="text-muted-foreground">معالج بواسطة</p>
                                        <p className="font-medium">{selectedRequest.processedBy.fullName}</p>
                                    </div>
                                )}
                            ...
                            {selectedRequest.customerNotes && (
                                <div>
                                    ...
                                    <p className="text-sm bg-muted p-3 rounded-md">{selectedRequest.customerNotes}</p>
                                </div>
                            )}
                            {selectedRequest.adminNotes && (
                                <div>
                                    <p className="text-muted-foreground mb-2">ملاحظات المشرف</p>
                                    <p className="text-sm bg-muted p-3 rounded-md">{selectedRequest.adminNotes}</p>
                                </div>
                            )}
                        </div>
                    )}
            ...
            <Dialog open={isProcessDialogOpen} onOpenChange={setIsProcessDialogOpen}>
                <DialogContent>
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            سيتم توليد كلمة مرور جديدة للعميل. يرجى نسخها وإرسالها للعميل يدوياً.
                        </DialogDescription>
                    </DialogHeader>
                            ...
                            <>
                                <div className="space-y-2">
                                    ...
                                    <p className="text-xs text-muted-foreground">
                                        يرجى نسخ كلمة المرور وإرسالها للعميل عبر واتساب أو إيميل أو هاتف.
                                    </p>
                                </div>
                                <DialogFooter>
                                    <Button
                ...
                <DialogContent>
                    ...
                    <form onSubmit={rejectForm.handleSubmit(onRejectSubmit)} className="space-y-4">
                        <div>
                            ...
                            <Textarea
                                id="rejectionReason"
                                {...rejectForm.register('rejectionReason', { required: 'سبب الرفض مطلوب' })}
