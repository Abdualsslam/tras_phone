...
export class SettingsController {
    ...
    async calculateTax(@Body() data: { amount: number; countryId?: string; categoryId?: string }) {
        const tax = await this.settingsService.calculateTax(data.amount, data.countryId, data.categoryId);
    ...
    async calculateShipping(@Body() data: { cityId: string; weight: number; orderAmount: number }) {
        const rates = await this.settingsService.calculateShipping(data.cityId, data.weight, data.orderAmount);
        return ResponseBuilder.success(rates);
    }
    ...
    async getPaymentMethods(@Query('platform') platform?: string) {
        const methods = await this.settingsService.findActivePaymentMethods(platform);
    ...
    async updateSetting(
        ...
        const setting = await this.settingsService.updateSetting(key, data.value, user.adminId);
    ...
    async updateMultipleSettings(
        ...
        await this.settingsService.updateMultipleSettings(data.settings, user.adminId);
    ...
    async createShippingZone(@CurrentUser() user: any, @Body() data: any) {
        const zone = await this.settingsService.createShippingZone(data, user.adminId);
    ...
    async updatePaymentMethod(
        ...
        const method = await this.settingsService.updatePaymentMethod(id, data, user.adminId);
