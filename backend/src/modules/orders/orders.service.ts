...
export class OrdersService {
    constructor(
        ...
        @InjectModel(OrderNote.name) private orderNoteModel: Model<OrderNoteDocument>,
    ...
    async createOrder(customerId: string, data: any): Promise<OrderDocument> {
        ...
        await this.recordStatusChange(order._id.toString(), '', 'pending', 'Order created', undefined, true);
     ...
     */
    async findAll(filters?: any): Promise<{ data: OrderDocument[]; total: number }> {
        const {
            page = 1,
            limit = 20,
            customerId,
     ...
     */
    async findById(id: string): Promise<{ order: OrderDocument; items: OrderItemDocument[]; history: OrderStatusHistoryDocument[] }> {
        ...
        const [items, history] = await Promise.all([
            this.orderItemModel.find({ orderId: order._id }).populate('productId', 'name nameAr mainImage'),
            this.statusHistoryModel.find({ orderId: order._id }).sort({ createdAt: 1 }),
        ]);
     ...
     * Update order status
     */
    async updateStatus(orderId: string, newStatus: string, userId?: string, notes?: string): Promise<OrderDocument> {
        ...
        // Record history
        await this.recordStatusChange(orderId, oldStatus, newStatus, notes, userId, false);
     ...
     */
    async updateShipmentStatus(shipmentId: string, status: string, trackingNumber?: string): Promise<ShipmentDocument> {
        const updateData: any = { status };

        if (trackingNumber) updateData.trackingNumber = trackingNumber;

        switch (status) {
            case 'picked': updateData.pickedAt = new Date(); break;
            case 'packed': updateData.packedAt = new Date(); break;
    ...
    async recordPayment(orderId: string, data: any): Promise<PaymentDocument> {
        ...
        await this.invoiceModel.findOneAndUpdate(
            { orderId },
            { $set: { paidAmount: newPaidAmount, status: paymentStatus, paidDate: new Date() } },
        );
     ...
     * Add note to order
     */
    async addNote(orderId: string, content: string, type: string, userId?: string): Promise<OrderNoteDocument> {
