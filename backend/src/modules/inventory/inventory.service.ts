...
import { ProductStock, ProductStockDocument } from './schemas/product-stock.schema';
import { StockMovement, StockMovementDocument } from './schemas/stock-movement.schema';
import { StockReservation, StockReservationDocument } from './schemas/stock-reservation.schema';
import { LowStockAlert, LowStockAlertDocument } from './schemas/low-stock-alert.schema';
...
export class InventoryService {
     ...
     */
    async getStock(productId: string, warehouseId: string): Promise<ProductStockDocument> {
        let stock = await this.productStockModel.findOne({ productId, warehouseId });
    ...
    async adjustStock(data: {
        ...
        const isInbound = [
            'purchase_in', 'sale_return', 'transfer_in',
            'adjustment_in', 'assembly_in',
        ].includes(data.movementType);

        const quantityChange = isInbound ? Math.abs(data.quantity) : -Math.abs(data.quantity);
    ...
    async reserveStock(data: {
        ...
        const available = await this.getAvailableQuantity(data.productId, data.warehouseId);
    ...
    async checkLowStock(productId: string, warehouseId: string): Promise<void> {
        const stock = await this.productStockModel.findOne({ productId, warehouseId });
     ...
     * Get stock movements
     */
    async getMovements(filters: any): Promise<{ data: StockMovementDocument[]; total: number }> {
        const { page = 1, limit = 50, productId, warehouseId, movementType, startDate, endDate } = filters;
