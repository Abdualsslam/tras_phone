...
export class TicketsController {
  ...
  async getMyTickets(@CurrentUser() user: any) {
    const tickets = await this.ticketsService.findCustomerTickets(
      user.customerId,
    );
  ...
  async createTicket(@CurrentUser() user: any, @Body() data: any) {
    ...
    const customerName =
      user.name ?? customerData?.name ?? user.phone ?? 'عميل';
  ...
  async rateTicket(
    ...
    const updated = await this.ticketsService.addSatisfactionRating(
      id,
      data.rating,
      data.feedback,
    );
  ...
  @Roles(UserRole.ADMIN)
  @ApiOperation({
    summary: 'Get ticket details (admin) - GET /support/tickets/:id',
  })
  ...
  @Roles(UserRole.ADMIN)
  @ApiOperation({
    summary: 'Update ticket status - PUT /support/tickets/:id/status',
  })
  ...
  @Roles(UserRole.ADMIN)
  @ApiOperation({
    summary: 'Reply to ticket - POST /support/tickets/:id/reply',
  })
  async replyToTicketById(
    @CurrentUser() user: any,
    @Param('id') id: string,
    @Body()
    data: { content: string; isInternal?: boolean; attachments?: string[] },
  ) {
    const message = await this.ticketsService.addMessage(id, {
  ...
  async escalateTicket(
    ...
    const ticket = await this.ticketsService.escalateTicket(
      id,
      user.adminId,
      data.reason,
    );
